...
export const Route = createFileRoute("/_authenticated/hanger/personal")({
  ...
  head: () => ({
    meta: [
      { title: "Personal Hanger — StickTime FPV" },
      {
        name: "description",
        content:
...
function Garage() {
  ...
  const addDeepLink = addParam === "1" || (addParam as unknown) === 1;
  ...
  // own cache entry so realtime events invalidate only what changed. Reads
  // run through the delta-sync layer: warm visits transfer 0 gear rows.
  ...
  const addGear = useMutation({
    mutationFn: async () => {
      ...
      if (error) throw error;
      // Fast-path the new row into the delta-sync cache so warm reads see it
      // immediately without waiting for a refetch.
      primeDeltaSyncCache(tableName, profile?.id ?? "", {
    ...
    onSuccess: (_data, variables) => {
      ...
      if (removed > 0 && variables.previousCount !== undefined) {
        ...
        toast.success(
          ...
          {
            description:
              "Packs with recorded IR readings are kept and come back if you undo.",
            action: {
              label: "Undo",
  ...
  const removePart = useMutation({
    mutationFn: async (id: string) => {
      ...
      const partTables = ["transmitter_parts", "goggles_parts", "other_parts"];
  ...
  return (
    <>
      <PageHeader
        ...
        action={
          <Dialog
            open={gearOpen || addDeepLink}
            onOpenChange={handleGearDialogChange}
          >
                ...
                <div className="overflow-hidden">
                  {items.length === 0 ? (
                    <div className="hud-panel p-8 text-center text-xs text-muted-foreground/70 border-dashed border-primary/20 bg-card/20 rounded-xl my-2">
                      No {TYPE_LABELS[section.key].toLowerCase()} registered
                      yet. Click the button above to add your equipment.
                    </div>
                  ) : (
