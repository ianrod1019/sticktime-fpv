...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Session+gear enrichment joins several tables per call: 30/min per user.
    const rateLimited = await checkRateLimit(supabase, "get-sessions-with-gear", user.id, 30, 60);
