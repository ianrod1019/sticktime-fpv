-- Returns sessions for a user with enriched gear data (drone, transmitter, goggles names)
create or replace function public.get_user_sessions_with_gear(p_user_id uuid, p_session_ids uuid[] default null)
returns table (
  id uuid,
  user_id uuid,
...
grant execute on function public.get_user_sessions_with_gear(uuid, uuid[]) to authenticated;