/*
    # Strict Secure RLS Policies for public.profiles

    1. Overview
      - Enables Row Level Security (RLS) on `public.profiles`.
      - **SELECT**: Users can read their own profile row. Admins and devs can read all profile rows.
      - **UPDATE**: Regular users are completely blocked from updating profiles directly from the client. Only admins and devs can perform updates.
  ...
  */
  ...
  -- 3. SELECT Policy: Users can view their own profile; admins/devs can view all
  ...
  -- 4. UPDATE Policy: Regular users cannot update profiles; only admin/dev roles can update
